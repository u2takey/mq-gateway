...
func request_{{.Method.Service.GetName}}_{{.Method.GetName}}_{{.Index}}(ctx context.Context, marshaler runtime.Marshaler, client {{.Method.Service.GetName}}Client, req []byte, pathParams map[string]string) ({{.Method.Service.GetName}}_{{.Method.GetName}}Client, error)
{{else}}
func request_{{.Method.Service.GetName}}_{{.Method.GetName}}_{{.Index}}(ctx context.Context, marshaler runtime.Marshaler, client {{.Method.Service.GetName}}Client, req []byte, pathParams map[string]string) (proto.Message, error)
{{end}}`, "\n", "", -1)))
...
{{template "request-func-signature" .}} {
	var protoReq {{.Method.RequestType.GoType .Method.Service.File.GoPkg.Path}}
{{if .Body}}
	if err := marshaler.NewDecoder(bytes.NewReader(req)).Decode(&{{.Body.AssignableExpr "protoReq"}}); err != nil && err != io.EOF  {
...
{{- end}}
	...
	if !ok {
		return nil, status.Errorf(codes.InvalidArgument, "missing parameter %s", {{$param | printf "%q"}})
	}
...
{{template "request-func-signature" .}} {
	...
	dec := marshaler.NewDecoder(bytes.NewReader(req))
...
func Register{{$svc.GetName}}{{$.RegisterFuncSuffix}}Client(ctx context.Context, mux *runtime.ServeMux, client {{$svc.GetName}}Client) error {
	{{range $m := $svc.Methods}}
	{{range $b := $m.Bindings}}
	mux.Handle({{$b.HTTPMethod | printf "%q"}} + "_{{$b.PathTmpl.Template}}", func(r []byte)(out []byte) {
		inboundMarshaler, outboundMarshaler := &runtime.JSONPb{OrigName: true}, &runtime.JSONPb{OrigName: true}
		resp, _ := request_{{$svc.GetName}}_{{$m.GetName}}_{{$b.Index}}(ctx, inboundMarshaler, client, r, nil)
